...
export function prettyPrintEntry(timeobj) {
	...
	if (timeobj.startTime && timeobj.endTime) {
		time = makeTimeString(pendel(timeobj.startTime, timeobj.endTime));
	...
	return `  ${chalk.white(timeobj.date)} ${chalk.cyan(to24(timeobj.startTime))} - ${chalk.gray('[clockout pending]')}\n`;
}
